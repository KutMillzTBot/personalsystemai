...
function syncDragLinesFromInputs() {
  ...
  setY("drag-entry-line", lastPrice);
  setY("drag-sl-line", Number($("manual-sl")?.value || lastPrice));
  setY("drag-tp-line", Number($("manual-tp")?.value || lastPrice));
}
...
async function placeManualTrade(side) {
  ...
  if (!res || res.status !== "ok") {
    addLog("Manual trade rejected. Check SL/TP placement.", "error");
    return;
  }

  addLog(`Manual ${sideNorm} opened on ${currentSymbol} ticket=${res.ticket}`, "trade");
  await refreshAll();
}
...
async function loadStatus() {
  ...
  if (backendSymbol) {
    if (autoSymbolSync && backendSymbol !== currentSymbol) {
    ...
    set("settings-mt-symbol", backendSymbol);
  } else {
    set("status-symbol", currentSymbol);
  }
  ...
  if (health) {
    ...
    const countForSymbol = Number(ingestCounts[backendSymbol || currentSymbol] || 0);
