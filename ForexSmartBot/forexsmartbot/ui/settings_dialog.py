...
class SettingsDialog(QDialog):
    ...
    def create_broker_tab(self) -> QWidget:
        ...
        self.broker_mode_combo.addItems(['PAPER', 'MT5', 'REST API', 'IB TWS'])
        layout.addRow(f"{self.tr('broker_mode', 'Broker Mode')}:", self.broker_mode_combo)
        
        # MT5 settings group
        mt4_group = QGroupBox("MT5 / Shared Bridge")
        ...
        self.mt4_port_spin.setValue(5050)
    ...
    def create_data_providers_tab(self) -> QWidget:
        ...
        # MT5 Data Provider group
        mt4_data_group = QGroupBox("MT5 Data Provider")
        mt4_data_layout = QFormLayout(mt4_data_group)
        
        self.mt4_data_enabled_checkbox = QCheckBox("Use MT5 bridge as Data Provider")
        mt4_data_layout.addRow(f"{self.tr('status', 'Status')}:", self.mt4_data_enabled_checkbox)
        
        mt4_data_info = QLabel("Use the shared MT5 bridge for live prices, account sync, watchlist data, and historical candles.")
    ...
    def load_settings(self):
        ...
        self.mt4_host_edit.setText(self.settings_manager.get('mt5_host', self.settings_manager.get('mt4_host', '127.0.0.1')))
        self.mt4_port_spin.setValue(self.settings_manager.get('mt5_port', self.settings_manager.get('mt4_port', 5050)))
        ...
        # Data Provider settings
        self.mt4_data_enabled_checkbox.setChecked(self.settings_manager.get('mt5_data_enabled', self.settings_manager.get('mt4_data_enabled', True)))
    ...
    def save_settings(self):
        ...
        try:
            ...
            if broker_mode == 'MT5':
                if not self.mt4_host_edit.text().strip() or self.mt4_port_spin.value() == 0:
                    QMessageBox.warning(self, "Broker Configuration", 
                                      "You need to configure the MT5 bridge settings before save")
                    return
            ...
            self.settings_manager.set('mt5_host', self.mt4_host_edit.text())
            self.settings_manager.set('mt5_port', self.mt4_port_spin.value())
            ...
            self.settings_manager.set('mt5_data_enabled', self.mt4_data_enabled_checkbox.isChecked())
    ...
    def test_data_providers(self):
        ...
        try:
            from ..adapters.data import MT5Provider
            ...
            if self.mt4_data_enabled_checkbox.isChecked():
                try:
                    ...
                    mt4_provider = MT5Provider(mt4_host, mt4_port)
                    
                    # Test if MT5/shared bridge is available
                    if mt4_provider.is_available():
                        results.append("✅ MT5 Data Provider: Available")
                    else:
                        results.append("❌ MT5 Data Provider: Not available (bridge not running or wrong host/port)")
                        
                except Exception as e:
                    results.append(f"❌ MT5 Data Provider: {str(e)}")
    ...
    def test_broker_connection(self):
        ...
        try:
            if broker_mode == 'PAPER':
                ...
                from ..adapters.brokers.mt5_broker import MT5Broker
