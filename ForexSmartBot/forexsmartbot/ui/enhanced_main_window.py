...
from ..adapters.brokers import PaperBroker, MT5Broker, RestBroker, IBTWSBroker
...
class EnhancedMainWindow(QMainWindow):
    ...
    def initialize_broker(self):
        ...
        try:
            ...
            # MT5 mode - use shared bridge broker
            if broker_mode == 'MT5':
                self.append_log("MT5 mode detected - using shared Supervisor bridge broker")
            ...
            elif broker_mode == 'MT5':
                # Check if MT5 bridge settings are configured (from settings or env vars)
                import os
                mt4_host = self.settings_manager.get('mt5_host')
                if not mt4_host:
                    mt4_host = os.getenv('MT5_BRIDGE_HOST', os.getenv('MT4_ZMQ_HOST', '127.0.0.1'))
                
                mt4_port = self.settings_manager.get('mt5_port')
                if not mt4_port or mt4_port == 0:
                    mt4_port = int(os.getenv('MT5_BRIDGE_PORT', os.getenv('MT4_ZMQ_PORT', '5050')))
                
                self.append_log(f"MT5 bridge settings - Host: {mt4_host}, Port: {mt4_port}")
                if not mt4_host or mt4_port == 0:
                    self.append_log("MT5 bridge settings not configured - using paper broker")
                    self.broker = PaperBroker(10000.0)
                else:
                    from forexsmartbot.adapters.brokers.mt5_broker import MT5Broker
                    self.broker = MT5Broker(host=mt4_host, port=mt4_port)
                    self.append_log(f"MT5 broker initialized: {mt4_host}:{mt4_port}")
            ...
            # Initialize data provider (MT5 bridge first, then fallback to others)
        ...
        try:
            if not self.is_connected:
                ...
                if self.broker and self.broker.connect():
                    ...
                    # Get real balance from MT5 bridge
                    try:
                        ...
                        if real_balance:
                            ...
                            self.append_log("Failed to get real balance: No response from MT5 bridge")
                    except Exception as e:
                        self.append_log(f"Failed to get real balance: {e}")
                    
                    # Sync positions from MT5 bridge
                    try:
                        ...
                        if mt4_positions:
                            ...
                            self.append_log(f"Synced {len(self.positions)} positions from MT5 bridge")
                            self.update_positions_display()
                    except Exception as e:
                        self.append_log(f"Failed to sync positions from MT5 bridge: {e}")
                else:
    ...
    def _get_current_balance(self):
        ...
        try:
            # Get real balance from MT5 bridge broker
    ...
    def create_position_from_signal(self, symbol, signal, price):
        ...
        try:
            ...
            # Use real price from MT5 bridge broker
            if self.broker and self.broker.is_connected():
                real_price = self.broker.get_price(symbol)
                if real_price is not None:
                    entry_price = real_price
                    self.append_log(f"Using real MT5 price for {symbol}: {entry_price}")
                else:
                    entry_price = price
            ...
            if self.broker and self.broker.is_connected():
                ...
                if order_id:
                    self.append_log(f"Order submitted to MT5: {order_id} for {symbol}")
                    ...
                    self.append_log(f"Failed to submit order to MT5 for {symbol}")
            else:
    ...
    def _sync_mt4_position_after_order(self, symbol: str, order_id: str):
        """Sync MT5 position after order submission (non-blocking callback)."""
        try:
            # Sync positions from MT5 bridge to get the actual order details
            if self.broker and self.broker.is_connected():
                mt4_positions = self.broker.get_positions()
                if mt4_positions and symbol in mt4_positions:
                    mt4_pos = mt4_positions[symbol]
                    # Use actual MT5 entry price, SL, and TP
                    ...
                    self.append_log(f"MT5 order confirmed: {symbol} at {entry_price:.4f}, SL={stop_loss:.4f if stop_loss else 'None'}, TP={take_profit:.4f if take_profit else 'None'}")
                else:
                    self.append_log(f"Warning: Order submitted but position not found in MT5 yet for {symbol}")
            else:
                self.append_log(f"Broker not connected - cannot verify order for {symbol}")
        except Exception as e:
            self.append_log(f"Warning: Could not verify order in MT5: {e}")
            ...
            for i, pos in enumerate(self.positions):
                if pos.get('symbol') == symbol:
                    ...
                    if self.broker and self.broker.is_connected() and 'order_id' in pos:
                        ...
                        if close_success:
                            self.append_log(f"Order closed in MT5 for {symbol}")
                        else:
                            self.append_log(f"Failed to close order in MT5 for {symbol}")
    ...
    def update_real_time_data(self):
        ...
        try:
            ...
            # Sync positions from MT5 broker if connected
            if self.broker and self.broker.is_connected():
                try:
                    ...
                    self.append_log(f"Error syncing MT5 data: {str(e)}")
