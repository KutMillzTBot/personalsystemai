...
class MainWindow(QMainWindow):
    def __init__(self, cfg: AppConfig):
        ...
        if self.broker_mode == "MT5":
            self.mt5 = MT5Bridge(cfg.mt5_host, cfg.mt5_port)
        ...
        self.broker_sel.addItems(["PAPER","MT5"])
    ...
    def on_connect(self):
        ...
        if mode == "MT5":
            if self.mt5 is None:
                self.mt5 = MT5Bridge(self.cfg.mt5_host, self.cfg.mt5_port)
            self.append_log("Connected to MT5 bridge.")
        else:
    ...
    def on_tick(self):
        ...
        if self.broker_mode == "PAPER":
            ...
            self.append_log(f"Signal {sig} on {pair} @ {price:.5f} (MT5 shared bridge live flow active)")
