...
class SettingsManager:
    ...
    def _load_settings(self) -> Dict[str, Any]:
        """Load settings from file, with environment variable fallback."""
        ...
        default_settings = {
            "theme": "auto",
            "language": "en",
            "default_symbols": os.getenv("SYMBOLS", "EURUSD,USDJPY,GBPUSD").split(","),
            "broker_mode": os.getenv("BROKER", "PAPER").upper(),
            ...
            "data_provider": "mt5",
            "data_interval": "1h",
            "mt5_data_enabled": True,
            "mt4_data_enabled": True,
            "strategy": "SMA_Crossover",
            "strategy_params": {},
            "portfolio_mode": False,
            "selected_symbols": os.getenv("SYMBOLS", "EURUSD").split(",") if os.getenv("SYMBOLS") else ["EURUSD"]
        }
        ...
        if self.settings_file.exists():
            try:
                with open(self.settings_file, 'r') as f:
                    ...
                    # Settings file values override env var defaults
                    default_settings.update(loaded)
            except Exception:
                pass
        
        # Environment variables take precedence over file settings for critical config
        # This allows runtime override via .env file
        if os.getenv("BROKER"):
            default_settings["broker_mode"] = os.getenv("BROKER", "PAPER").upper()
        if os.getenv("MT5_BRIDGE_HOST") or os.getenv("MT4_ZMQ_HOST"):
            bridge_host = os.getenv("MT5_BRIDGE_HOST", os.getenv("MT4_ZMQ_HOST", "127.0.0.1"))
            default_settings["mt5_host"] = bridge_host
            default_settings["mt4_host"] = bridge_host
        if os.getenv("MT5_BRIDGE_PORT") or os.getenv("MT4_ZMQ_PORT"):
            bridge_port = int(os.getenv("MT5_BRIDGE_PORT", os.getenv("MT4_ZMQ_PORT", "5050")))
            default_settings["mt5_port"] = bridge_port
            default_settings["mt4_port"] = bridge_port
