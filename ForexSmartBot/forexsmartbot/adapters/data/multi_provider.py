    ...
    def __init__(self, providers: List[IDataProvider] = None, settings_manager=None):
        ...
        if providers is None:
            ...
            if self.settings_manager:
                ...
                # Add MT5/shared bridge provider first if enabled
                if self.settings_manager.get('mt5_data_enabled', self.settings_manager.get('mt4_data_enabled', False)):
                    mt5_host = self.settings_manager.get('mt5_host', self.settings_manager.get('mt4_host', '127.0.0.1'))
                    mt5_port = self.settings_manager.get('mt5_port', self.settings_manager.get('mt4_port', 5050))
                    mt5_provider = MT5Provider(mt5_host, mt5_port)
                    self.providers.append(mt5_provider)
                    print("Added MT5Provider as primary provider")
